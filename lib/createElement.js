...
DOMRenderer.prototype = {
  mergeProps(props = {}) {
    ...
    const finalAttrList = this.type === ElementTypes.SVG
      ? mergedAttrList
    ...
    children.filter(childNode => isDefined(childNode))
      .forEach(childNode => {
        if (childNode instanceof HTMLElement || childNode instanceof SVGElement) {
          ...
          throw new Error(`Expected "object" or "string" but instead got ${typeof childNode}`)
        }
      }
    )
    return this
...
const concatStringAttr = (attrNameList, attrCollection) => {
  return attrCollection.map(group => {
    return attrNameList.map(key => group[key]).filter(val => val).join()
  }).join(' ').trim()
}
...
const createAttrMap = node => {
  return node.attributes && Array.from(node.attributes).reduce((acc, cur, i) => {
...
const setNodeAttr = (node, attrList) => {
  Object.keys(attrList).filter(k => attrList[k]).map(p => {
