...
// Replaces the root <template> element if found
const composeContent = content => {
...
const renderToHost = async (content, host) => {
  const resolvedContent = composeContent(content)
...
export default async (content, host) => {
  try {
    await renderToHost(content, host)
  } catch (e) {
